...
@section('content')
    ...
    <div class="page-title-section flex items-center justify-between mb-6">
        <div>
            ...
            <p class="text-gray-500 text-xs md:text-sm mt-1">
                Welcome back, {{ Auth::user()->name ?? 'Guest' }}! Here's your business summary.
            </p>
        </div>
    ...
    <div class="stats-grid grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <div class="bg-white rounded-xl p-4 stat-card border border-gray-100 shadow-sm animate-slide-in">
            ...
            <h3 class="text-2xl font-bold text-gray-800 mb-2">${{ number_format(54239, 2) }}</h3>
        ...
        <div class="bg-white rounded-xl p-4 stat-card border border-gray-100 shadow-sm animate-slide-in" style="animation-delay: 0.1s;">
            ...
            <h3 class="text-2xl font-bold text-gray-800 mb-2">{{ number_format(2842) }}</h3>
            <div class="flex items-center justify-between text-xs">
        ...
        <div class="bg-white rounded-xl p-4 stat-card border border-gray-100 shadow-sm animate-slide-in" style="animation-delay: 0.2s;">
            ...
            <h3 class="text-2xl font-bold text-gray-800 mb-2">{{ number_format(1482) }}</h3>
            <div class="flex items-center justify-between text-xs">
            ...
            <div class="h-56 flex items-end justify-between space-x-2">
                ...
                @foreach($months as $index => $month)
                    <div class="flex flex-col items-center flex-1 group">
                        <div class="w-full primary-color rounded-t-lg hover:opacity-90 cursor-pointer transition" style="height: {{ $heights[$index] }}%"></div>
                        <span class="text-xs text-gray-600 mt-2 font-medium">{{ $month }}</span>
                    </div>
                @endforeach
            </div>
        </div>
                ...
                @foreach($products as $product)
                    <div>
                        <div class="flex items-center justify-between mb-1">
                            ...
                            <span class="text-xs text-gray-500">{{ $product['sales'] }} sales</span>
                        </div>
                        ...
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="py-3 px-3">
                                <div class="flex items-center">
                                    <img src="https://ui-avatars.com/api/?name={{ urlencode($order['customer']) }}&background=random&color=fff&font-size=0.4" 
                                         alt="{{ $order['customer'] }}" 
                            ...
                            <td class="py-3 px-3 text-sm text-gray-600">{{ $order['date'] }}</td>
                            <td class="py-3 px-3 text-sm font-semibold text-gray-800">${{ number_format($order['amount'], 2) }}</td>
                            <td class="py-3 px-3">
                                <span class="inline-flex items-center px-2 py-1 text-xs font-medium rounded-md 
                                    @if($order['status_color'] == 'green') bg-green-50 text-green-700
                                    @elseif($order['status_color'] == 'yellow') bg-yellow-50 text-yellow-700
                                    @elseif($order['status_color'] == 'blue') bg-blue-50 text-blue-700
                                    @elseif($order['status_color'] == 'red') bg-red-50 text-red-700
                                    @endif">
