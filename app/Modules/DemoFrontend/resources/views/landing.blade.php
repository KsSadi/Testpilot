...
@section('title', ($appName ?? config('app.name')) . ' - Modern Laravel Starter Kit')
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
                ...
                <div class="text-center lg:text-left">
                    <h1 class="text-4xl md:text-5xl lg:text-6xl font-bold text-gray-900 mb-6">
                        Welcome to
                        <span class="gradient-text">{{ $appName ?? config('app.name', 'LaraKit') }}</span>
                    </h1>
                    <p class="text-xl text-gray-600 mb-8">
                        {{ $appTagline ?? setting('app_tagline', 'Modern Laravel Starter Kit') }}
                    </p>
                    ...
                    <div class="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start">
                        <a href="{{ route('landing.docs') }}" class="px-8 py-4 gradient-bg text-white font-semibold rounded-lg shadow-lg hover:shadow-xl transition transform hover:-translate-y-1">
                            <i class="fas fa-book mr-2"></i> View Documentation
                        </a>
                        <a href="{{ url('/dashboard') }}" class="px-8 py-4 bg-white text-cyan-600 font-semibold rounded-lg shadow-lg hover:shadow-xl transition border-2 border-cyan-600 transform hover:-translate-y-1">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                @foreach($features as $feature)
                    <div class="bg-white rounded-xl p-6 shadow-lg hover:shadow-2xl transition-all duration-300 hover-scale border border-gray-100">
                        <div class="w-14 h-14 rounded-lg bg-gradient-to-br {{ $feature['color'] }} flex items-center justify-center mb-4">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-6">
                @foreach($authMethods as $method)
                    <div class="bg-gray-50 rounded-xl p-6 text-center border-2 {{ $method['enabled'] ? 'border-' . $method['color'] . '-500 bg-' . $method['color'] . '-50' : 'border-gray-200' }} transition-all">
                        <div class="inline-flex items-center justify-center w-16 h-16 rounded-full {{ $method['enabled'] ? 'bg-' . $method['color'] . '-100' : 'bg-gray-200' }} mb-4">
                            <i class="{{ $method['icon'] }} text-2xl {{ $method['enabled'] ? 'text-' . $method['color'] . '-600' : 'text-gray-400' }}"></i>
                        </div>
                        <h3 class="font-semibold text-gray-900 mb-2">{{ $method['name'] }}</h3>
                        <span class="inline-block px-3 py-1 rounded-full text-xs font-medium {{ $method['enabled'] ? 'bg-green-100 text-green-700' : 'bg-gray-200 text-gray-600' }}">
            ...
            <div class="text-center mt-12">
                ...
                <a href="{{ route('settings.index', ['tab' => 'auth']) }}" class="inline-flex items-center text-cyan-600 hover:text-cyan-700 font-medium">
    ...
    <section class="section-padding bg-gradient-to-br from-cyan-500 via-blue-500 to-purple-600 text-white">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            ...
            <p class="text-xl mb-8 opacity-90 max-w-3xl mx-auto">
                Get started with {{ $appName ?? config('app.name') }} and build your next Laravel application faster than ever.
            </p>
            <div class="flex flex-col sm:flex-row gap-4 justify-center">
                <a href="{{ route('landing.docs') }}" class="px-8 py-4 bg-white text-cyan-600 font-semibold rounded-lg shadow-lg hover:shadow-xl transition transform hover:-translate-y-1">
                    <i class="fas fa-book mr-2"></i> Read Documentation
                </a>
                <a href="{{ url('/dashboard') }}" class="px-8 py-4 bg-transparent border-2 border-white text-white font-semibold rounded-lg shadow-lg hover:bg-white hover:text-cyan-600 transition transform hover:-translate-y-1">
                    <i class="fas fa-rocket mr-2"></i> Try Dashboard
                </a>
            </div>
        </div>
