...
<div class="mt-6 pt-6 border-t border-gray-200">
    <div class="flex flex-col md:flex-row items-center justify-between text-xs md:text-sm text-gray-500 gap-4">
        <div class="flex flex-col sm:flex-row items-center space-y-2 sm:space-y-0 sm:space-x-4 text-center">
            <span>{{ $copyrightText ?? '© ' . date('Y') . ' ' . ($appName ?? config('app.name', 'Dashboard')) . '. All rights reserved.' }}</span>
            @if($footerText ?? null)
                <span class="hidden sm:inline">•</span>
                <span>{{ $footerText }}</span>
            @endif
            <span class="hidden sm:inline">•</span>
            <a href="{{ url('/privacy-policy') }}" class="text-cyan-600 hover:text-cyan-700 hover:underline transition">Privacy Policy</a>
            <span class="hidden sm:inline">•</span>
            <a href="{{ url('/terms-of-service') }}" class="text-cyan-600 hover:text-cyan-700 hover:underline transition">Terms of Service</a>
        </div>
